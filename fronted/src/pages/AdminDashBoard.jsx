...
const LoadingDots = ({ color = "indigo", size = "medium" }) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      <div
        className={`rounded-full ${sizeClasses[size]} bg-${color}-600 ${
...
const AdminDashboard = () => {
  ...
  const calculateAnalytics = (data) => {
    ...
    const totalAmount = data.reduce(
      (sum, tx) => sum + tx.transaction_amount,
      0
    );
    const fraudAmount = fraudTxs.reduce(
      (sum, tx) => sum + tx.transaction_amount,
      0
    );
  ...
  const toggleFraudStatus = async (transactionId, currentStatus) => {
    try {
      const response = await fetch(
        `http://13.127.98.0/api/transactions/${transactionId}/fraud`,
        {
          method: "PATCH",
  ...
  const pieChartData = [
    {
      name: "Normal",
      value:
        transactions.length - transactions.filter((tx) => tx.is_fraud).length,
    },
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 transition-all duration-300 hover:shadow-md">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800 mt-1">
                  {analytics.fraudPercentage.toFixed(1)}%
                </h3>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 transition-all duration-300 hover:shadow-md">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800 mt-1">
                  ₹{analytics.fraudAmount.toLocaleString()}
                </h3>
              </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-sm font-medium">
                          <div className="flex space-x-3">
                            ...
                            <button
                              onClick={() =>
                                toggleFraudStatus(tx._id, tx.is_fraud)
                              }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-gray-800">
                          {selectedTransaction.sender_lat},{" "}
                          {selectedTransaction.sender_long}
                        </p>
                      </div>
